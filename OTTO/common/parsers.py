...
def all_characteristics(soup: BeautifulSoup) -> dict[str, str]:
    """All PDP characteristic rows (dv_characteristicsTable) as {label: value}."""
    values: dict[str, str] = {}
    for row in soup.select(".dv_characteristicsTable tr"):
        cells = row.find_all("td")
        if len(cells) < 2:
...
def characteristic_by_label(soup: BeautifulSoup, *label_options: str) -> str | None:
    """Return the value of the first characteristic row whose label matches one of label_options."""
    rows = all_characteristics(soup)
    for label, value in rows.items():
        low = label.lower()
        if any(opt.lower() in low for opt in label_options):
            return value
    return None


def extract_recommendation_intent(soup: BeautifulSoup) -> str | None:
    """'<N>% würden diesen Artikel weiterempfehlen' -> '<N>% would recommend this article'."""
    text = text_clean(soup.get_text(" ", strip=True)) or ""
    match = re.search(r"(\d{1,3})\s*%\s+würden diesen Artikel\s+weiterempfehlen", text)
    return f"{match.group(1)}% would recommend this article" if match else None
...
def extract_review_rating(soup: BeautifulSoup) -> tuple[str | None, int | None]:
    """Per-variation star rating from the review page summary.

    Markup: `<div class="pdp_cr-rating-score"><span class="oc-headline-300">4.5</span>
    <span>von 5</span></div>` and the wrapper text reads "4.5 von 5 (177)" where 177 is
    the number of star ratings. Returns (average, count); (None, None) when the page has
    no rating block (e.g. a variation with zero ratings).
    """
    score = soup.select_one(".pdp_cr-rating-score .oc-headline-300")
    average = text_clean(score.get_text(" ", strip=True)) if score else None
    count = None
    wrapper = soup.select_one(".pdp_cr-rating")
    if wrapper:
        m = re.search(r"\(([\d.  ,]+)\)", wrapper.get_text(" ", strip=True))
        if m:
